...
* In the Kubernetes API, a _resource_ is an endpoint that stores a collection of API objects of a certain kind.
...
Make sure you can see the target S3 bucket.
It is in the PROD AWS account, but is also readable by the DR AWS account.
...
----
CAUTION: Please *_do not_* change that appvault-secret password as the same bucket is used for both production and DR environments to allow for disaster recovery/failover.
The AppVault is already configured with this password on the production environment
...
We will use the tridentctl-protect tool here, but you can achieve the same via declarative YAML manifests, ready to be included in your GitOps pipeline.
[.lines_space]
[.console-input]
[source,bash,role=execute,subs="attributes"]
----
